...
\begin{proposition}[Expectation-Maximization for Taxonomy Clustering model]
    ...
    \textbf{Expectation step}:
        ...
        p_{\widehat{\theta}}(Z_i = c | X_i, T_i) = \tau_{ic} =
                        \frac{\gamma_c p_{\widehat{\theta}}(T_i | Z_i = c) p_{\widehat{\theta}}(X_i | T_i, Z_i = c)
                             }
                             {
                                \sum_{c'=1}^C
                                \gamma_{c'} p_{\widehat{\theta}}(T_i | Z_i = c') p_{\widehat{\theta}}(X_i | T_i, Z_i = c')
                             }
        $$
        ...
        $$
        \begin{align}
            p_{\widehat{\theta}}(T_i | Z_i = c) &= \prod_{\ell=1}^{L-1} \prod_{k=1}^{K_{\ell}} \left[\left(\pi_{c,k}^{(\ell)}\right)^{u_{k,i}^{(\ell)}} \left(1 - \pi_{c,k}^{(\ell)}\right)^{(1 - u_{k,i}^{(\ell)})}\right]^{\nodeparent(u_{k,i}^{(\ell)})} \\
            p_{\widehat{\theta}}(X_i | T_i, Z_i = c) &= \prod_{\ell=1}^{L-1} \prod_{k=1}^{K_{\ell}} \delta_{x_k^{(\ell)}}\left(\childrennode(x_{k,i}^{(\ell)})\right)^{\mathds{1}_{|\childrennode(x_{k,i}^{(\ell)})|}=1} \left(\frac{1}{x_{k,i}^{(\ell)}} f_{\alpha_{c,k}^{(\ell)} \odot T_k^{(\ell+1)}}\left(\frac{\childrennode(x_{k,i}^{(\ell)})}{x_{k,i}^{(\ell)}}\right) \right)^{\mathds{1}_{|\childrennode(x_{k,i}^{(\ell)})|} > 1}
        \end{align}
        $$
